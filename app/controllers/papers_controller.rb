class PapersController < ApplicationController
  ...
  def show(id)
    ...
    @contents = Contents::Web.contents.sortable(params[:sort]).in(@paper.contents).page(params[:page])
  end
  def term(from_date, to_date)
    ...
    @contents = Contents::Web.contents.sortable(params[:sort]).gte(:created_at => from_time).lte(:created_at => to_time).page(params[:page])
