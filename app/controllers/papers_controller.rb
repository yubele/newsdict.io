class PapersController < ApplicationController
  ...
  def show(id)
    ...
    @contents = Contents::Web.exclude_domain.sortable(params[:sort]).in(@paper.contents).page(params[:page])
  end
  def term(from_date, to_date)
    ...
    @contents = Contents::Web.exclude_domain.sortable(params[:sort]).gte(:created_at => from_time).lte(:created_at => to_time).page(params[:page])
  ...
  def one_day(date)
    ...
    @paper = Paper.new
    @paper.user = User.find_by(email: ENV['ADMIN_USER_EMAIL'])
    @contents = Contents::Web.exclude_domain.sortable(params[:sort]).gte(:created_at => time.midnight).lte(:created_at => time.end_of_day).page(params[:page])
